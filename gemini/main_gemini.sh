...
for net in GeneMANIA_ex mashup_GeneMANIA_ex mashup_ex; do
    for org in yeast mouse human_match; do
        ...
        embed_type=Qsm4
        axis=1
        OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini_cluster.py --embed_type $embed_type --axis $axis --level network --cluster_method $cluster_method --separate $separate --run_mashup 0 --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
