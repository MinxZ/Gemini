...
for org in yeast mouse human_match; do
    for net in mashup_ex mashup_GeneMANIA_ex GeneMANIA_ex; do
        ...
        OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini.py --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
        if [ $org = yeast ]; then
            ndim=200
        else
            ndim=400
        fi
        python main_classifier.py --ndim $ndim --embed_name $embed_name --org $org --net $net --experiment_name $experiment_name
    done
done
...
for org in yeast mouse human; do
    ...
    OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini.py --mixup -1 --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
    if [ $org = yeast ]; then
        ndim=200
    else
        ndim=400
    ...
    for rwr in pca svd; do
        ...
        OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini.py --rwr $rwr --mixup -2 --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
        if [ $org = yeast ]; then
            ndim=200
        else
            ndim=400
...
for net in GeneMANIA_ex mashup_GeneMANIA_ex mashup_ex; do
    for org in yeast mouse human_match; do
        ...
        embed_type=Qsm4
        axis=1
        OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini_confi.py --embed_type $embed_type --axis $axis --level network --cluster_method $cluster_method --separate $separate --run_mashup 0 --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
        OPENBLAS_NUM_THREADS=$OPENBLAS_NUM_THREADS python main_gemini.py --ori_seed $ori_seed --mixup2 $mixup2 --gamma $gamma --embed_type $embed_type --mixup $mixup --axis $axis --level $level --cluster_method $cluster_method --ori_weight $ori_weight --weight $weight --separate $separate --num_thread $num_thread --torch_thread $torch_thread --method $method --org $org --net $net --ndim $ndim
        ...
        echo $experiment_name
        # python main_classifier.py --mixup $mixup --ndim $ndim --embed_name $embed_name --org $org --net $net --experiment_name $experiment_name
        ...
        for seed in 1; do
            ...
            echo $experiment_name
            python main_classifier.py --seed $seed --mixup $mixup --ndim $ndim --embed_name $embed_name --org $org --net $net --experiment_name $experiment_name
        done
    done
done
