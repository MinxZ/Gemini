"""
Learn embeddings for Gemini or Mashup with variable number of input networks,
keeping track of the runtime, peak CPU usage, and peak GPU usage.
"""
...
from gemini.func import out_string_nets, textread
from gemini.mashup import load_multi, mashup, mashup_multi
...
def main():
    ...
    print("---------STARTING TO MONITOR GEMINI.PY-----------------")
    ...
    embd_name = GEMINI_DIR + f'data/embed/{args.num_nets}--{method}_{org}_{net}_{ndim}'
    ...
    if args.weight > 0 and args.separate != 0:
        ...
        for embed_type in [args.embed_type]:
            ...
            for axis in iters:
                separate = np.load(
                    GEMINI_DIR + f'data/separate/{args.num_nets}--{net}_{org}_type0_' +
    ...
    import json
    with open('monitoring_results/{}_M={}_results.txt'.format(args.method, args.num_nets), 'w') as f:
